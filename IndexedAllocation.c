...
void insertblock(struct Index** head, int data, struct Node* link) {
...
void printBlock(struct Index* head) {
    struct Index* current = head;
    while (current != NULL) {
...
void chkblk(struct Index* head, int data){
    struct Index* current = head;
    while (current != NULL){
        if(current->data==data){
            printLink(current->link, data);
            current=NULL;
        }
        else{
            current = current->next;
...
void check(struct Node* head,struct Index** red, int curre, int total, int pg) {
    int length = getlen(head);
    srand(time(NULL));
    int startindx = rand() % length;
    struct Node* current = getnod(head, startindx);
    insertblock(red, pg, current);
    ...
    while (k) {
        ...
        if (k == 1) {
            ...
            if (memory->free>=size) {
                ...
                check(memory,&memo,size%totalsize,size,pg);
                printList(memory);
                printBlock(memo);
            } else {
